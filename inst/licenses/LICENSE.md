...
The `ColombiAPI` package also depends on two additional public RESTful APIs:
...
1. **World Bank Open Data API**
   ...
   In the `ColombiAPI` package, the API is used to **dynamically retrieve and display real-time indicators for Colombia**. Data is accessed live via HTTP requests to the API and shown directly in the R console. **No data is stored or redistributed** by this package.
